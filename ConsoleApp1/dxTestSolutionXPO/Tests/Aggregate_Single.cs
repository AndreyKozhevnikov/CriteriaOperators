...
namespace dxTestSolutionXPO.Tests {
    ...
    public class Aggregate_Single : BaseTest {
        ...
        public void Task3_0() {
            ...
            var result3 = new XPCollection<OrderItem>(uow, criterion);
            //assert
        ...
        public void Task3_1() {
            ...
            CriteriaOperator resCrit = new BinaryOperator(nameof(OrderItem.Order), joinCriterion);
            var result3 = new XPCollection<OrderItem>(uow, resCrit);
            //assert
            Assert.AreEqual(4, result3.Count);
        }
        [Test]
        public void Task3_2() {
            ...
            var uow = new UnitOfWork();
            //act
            CriteriaOperator resCrit = CriteriaOperator.FromLambda<OrderItem>(oi => oi.Order == FromLambdaFunctions.FreeJoin<Order>(o => o.OrderItems.Any(oii => oii.ItemPrice == 456)).SingleOrDefault());
            var result3 = new XPCollection<OrderItem>(uow, resCrit);
            //assert
            Assert.AreEqual(4, result3.Count);
        }
